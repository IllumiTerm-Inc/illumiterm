...
void NameTab(GtkWindow *main_window) {
    GtkWidget *dialog = gtk_dialog_new_with_buttons("Name Tab", main_window, GTK_DIALOG_MODAL, "Cancel", GTK_RESPONSE_CANCEL, "OK", GTK_RESPONSE_OK, NULL);
    ...
    if (result == GTK_RESPONSE_OK) {
        ...
        gtk_window_set_title(main_window, name);
    }
...
gboolean HandleKeyPress(GtkWidget *widget, GdkEventKey *event, gpointer window)
{
    ...
    if ((event->state & GDK_SHIFT_MASK) && (event->state & GDK_CONTROL_MASK)) {
        switch (gdk_keyval_to_lower(event->keyval)) {
            ...
            case GDK_KEY_i: NameTab(window); return TRUE;
...
void AboutWindow(GtkWindow* parent) {
    ...
    const gchar* authors[] = {"Elijah Gordon", "<a href=\"mailto:nitrixxero@gmail.com\">nitrixxero@gmail.com</a>", NULL};
